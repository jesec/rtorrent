...
ObjectStorageTest::test_conversions() {
  CPPUNIT_ASSERT(m_storage.insert("test_1",
                                  torrent::Object("a"),
                                  rpc::object_storage::flag_string_type) !=
                 m_storage.end());
  CPPUNIT_ASSERT(m_storage.insert_str(std::string("test_2"),
                                      torrent::Object("a"),
                                      rpc::object_storage::flag_string_type) !=
                 m_storage.end());
...
ObjectStorageTest::test_access() {
  ...
  CPPUNIT_ASSERT(m_storage.set_c_str_string("string_1", "test").as_string() ==
                 "test");

  // Test value from raw and normal, list, etc.
  CPPUNIT_ASSERT(m_storage.get_c_str("value_1").as_value() == 1);
  CPPUNIT_ASSERT(m_storage.set_c_str_value("value_1", int64_t(2)).as_value() ==
                 2);
  //  CPPUNIT_ASSERT(m_storage.set_c_str_value("value_1", "123").as_value() ==
  //  123); CPPUNIT_ASSERT(m_storage.set_c_str_value("value_1",
  //  torrent::raw_string::from_c_str("321")).as_value() == 321);
  //  CPPUNIT_ASSERT(m_storage.set_c_str_value("value_1",
