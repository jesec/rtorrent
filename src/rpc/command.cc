...
#define COMMAND_BASE_TEMPLATE_DEFINE(func_name)                                \
  template const torrent::Object func_name<target_type>(                       \
    command_base * rawCommand,                                                 \
    target_type            target,                                             \
    const torrent::Object& args);                                              \
  template const torrent::Object func_name<core::Download*>(                   \
    command_base * rawCommand,                                                 \
    target_type            target,                                             \
    const torrent::Object& args);                                              \
  template const torrent::Object func_name<torrent::Peer*>(                    \
    command_base * rawCommand,                                                 \
    target_type            target,                                             \
    const torrent::Object& args);                                              \
  template const torrent::Object func_name<torrent::Tracker*>(                 \
    command_base * rawCommand,                                                 \
    target_type            target,                                             \
    const torrent::Object& args);                                              \
  template const torrent::Object func_name<torrent::File*>(                    \
    command_base * rawCommand,                                                 \
    target_type            target,                                             \
    const torrent::Object& args);                                              \
  template const torrent::Object func_name<torrent::FileListIterator*>(        \
    command_base * rawCommand,                                                 \
    target_type            target,                                             \
    const torrent::Object& args);
...
command_base_call(command_base*          rawCommand,
  ...
  return command_base::_call<typename command_function<T>::type, T>(
...
template<typename T>
const torrent::Object
command_base_call_value_base(command_base*          rawCommand,
                             target_type            target,
  ...
  if (arg.type() == torrent::Object::TYPE_STRING) {
    ...
    return command_base::_call<typename command_value_function<T>::type, T>(
      rawCommand, target, val);
  }

  return command_base::_call<typename command_value_function<T>::type, T>(
    rawCommand, target, unit * arg.as_value());
}

template<typename T>
const torrent::Object
command_base_call_value(command_base*          rawCommand,
                        target_type            target,
                        const torrent::Object& rawArgs) {
...
template<typename T>
const torrent::Object
command_base_call_value_kb(command_base*          rawCommand,
                           target_type            target,
                           const torrent::Object& rawArgs) {
...
template<typename T>
const torrent::Object
command_base_call_string(command_base*          rawCommand,
                         target_type            target,
                         const torrent::Object& rawArgs) {
  if (!is_target_compatible<T>(target))
    throw torrent::input_error("Target of wrong type to command.");
  ...
  if (arg.type() == torrent::Object::TYPE_RAW_STRING)
    return command_base::_call<typename command_string_function<T>::type, T>(
      rawCommand, target, arg.as_raw_string().as_string());
...
template<typename T>
const torrent::Object
command_base_call_list(command_base*          rawCommand,
                       target_type            target,
                       const torrent::Object& rawArgs) {
  if (!is_target_compatible<T>(target))
    throw torrent::input_error("Target of wrong type to command.");
  ...
  if (rawArgs.type() != torrent::Object::TYPE_LIST) {
    ...
    return command_base::_call<typename command_list_function<T>::type, T>(
      rawCommand, target, arg);
  }
