...
Download::adjust_down_throttle(int throttle) {
  ...
  if (itr == control->core()->throttles().end() || itr->second.second == NULL ||
      itr->first == "NULL")
...
Download::adjust_up_throttle(int throttle) {
  ...
  if (itr == control->core()->throttles().end() || itr->second.first == NULL ||
      itr->first == "NULL")
...
Download::bind_keys() {
  ...
  m_bindings['t'] = std::bind(
    &torrent::Download::manual_request, m_download->download(), false);
  m_bindings['T'] =
    std::bind(&torrent::Download::manual_request, m_download->download(), true);

  const char* keys = control->ui()->get_throttle_keys();

  m_bindings[keys[0]] =
    std::bind(&Download::adjust_up_throttle,
              this,
  ...
  m_bindings[keys[1]] =
    std::bind(&Download::adjust_up_throttle,
              ...
              (int)-rpc::call_command_value("ui.throttle.global.step.small"));
  m_bindings[keys[2]] =
    std::bind(&Download::adjust_down_throttle,
              this,
              (int)rpc::call_command_value("ui.throttle.global.step.small"));
  m_bindings[keys[3]] =
    std::bind(&Download::adjust_down_throttle,
