...
ElementDownloadList::ElementDownloadList() {
  ...
  m_bindings['\x04'] = [this] {
    receive_command("branch=d.state=,d.stop=,d.erase=");
  };
  m_bindings['\x0B'] = [this] {
    receive_command("d.ignore_commands.set=1; d.stop=; d.close=");
  };
  m_bindings['\x12'] = [this] {
    receive_command("d.complete.set=0; d.check_hash=");
  };
  m_bindings['\x05'] = [this] {
    receive_command("f.multicall=,f.set_create_queued=0,f.set_resize_queued=0; "
  ...
  m_bindings['B' - '@'] = [this] {
    receive_command(
      ...
      "{d.connection_seed.set=initial_seed, print=\"Enabled initial seeding "
      "for "
      "the selected download.\"}");
  };

  m_bindings['U'] = [this] {
    receive_command("d.delete_tied=; print=\"Cleared tied to file "
  ...
  m_bindings['L'] = [this] { toggle_layout(); };
}
