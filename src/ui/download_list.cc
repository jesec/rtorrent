...
DownloadList::unfocus_download(core::Download* d) {
  ...
  if (*current_view()->focus() == d &&
      current_view()->focus() < current_view()->end_visible())
  ...
  switch (displayType) {
    case DISPLAY_DOWNLOAD:
      ...
      if (current_view()->focus() == current_view()->end_visible()) {
        ...
        download->slot_exit(std::bind(
          &DownloadList::activate_display, this, DISPLAY_DOWNLOAD_LIST));
...
DownloadList::receive_view_input(Input type) {
  ...
  input->signal_show_next().push_back(
    std::bind(&DownloadList::activate_display, this, DISPLAY_STRING_LIST));
  input->signal_show_next().push_back(
    std::bind(&ElementStringList::next_screen, esl));

  input->signal_show_range().push_back(
    std::bind(&DownloadList::activate_display, this, DISPLAY_STRING_LIST));
  input->signal_show_range().push_back(
    std::bind(&ElementStringList::set_range_dirent<utils::Directory::iterator>,
              esl,
              std::placeholders::_1,
              std::placeholders::_2));
  ...
  input->bindings()[KEY_ENTER] =
    std::bind(&DownloadList::receive_exit_input, this, type);
  input->bindings()['\x07'] =
    std::bind(&DownloadList::receive_exit_input, this, INPUT_NONE); // ^G
  input->bindings()['\x1B'] =
    std::bind(&DownloadList::receive_exit_input, this, INPUT_NONE); // ESC , ^[
...
DownloadList::setup_keys() {
  m_bindings['\x7f'] =
    std::bind(&DownloadList::receive_view_input, this, INPUT_LOAD_DEFAULT);
  m_bindings[KEY_BACKSPACE] =
    std::bind(&DownloadList::receive_view_input, this, INPUT_LOAD_DEFAULT);
  m_bindings['\n'] =
    std::bind(&DownloadList::receive_view_input, this, INPUT_LOAD_MODIFIED);
  m_bindings[KEY_ENTER] =
    std::bind(&DownloadList::receive_view_input, this, INPUT_LOAD_MODIFIED);
  m_bindings['\x0F'] =
    std::bind(&DownloadList::receive_view_input, this, INPUT_CHANGE_DIRECTORY);
  m_bindings['X' - '@'] =
    std::bind(&DownloadList::receive_view_input, this, INPUT_COMMAND);
  m_bindings['F'] =
    std::bind(&DownloadList::receive_view_input, this, INPUT_FILTER);
