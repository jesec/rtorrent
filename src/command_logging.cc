...
log_add_group_output_str(const char* group_name, const char* output_id) {
  int log_group =
    torrent::option_find_string(torrent::OPTION_LOG_GROUP, group_name);
...
apply_log_open(int output_flags, const torrent::Object::list_type& args) {
  ...
  if ((output_flags & log_flag_use_gz))
    torrent::log_open_gz_file_output(
      output_id.c_str(), file_name.c_str(), append);
  else
...
apply_log(const torrent::Object::string_type& arg, int logType) {
  ...
  if (!arg.empty()) {
    int logFd = open(torrent::utils::path_expand(arg).c_str(),
    ...
    switch (logType) {
      case 0:
        ...
        break;
        //     case 1: if (control->scgi()) control->scgi()->set_log_fd(logFd);
        //     break;
      default:
        break;
    }
...
log_vmmap_dump(const std::string& str) {
  ...
  for (core::DownloadList::iterator itr = d_list->begin(), last = d_list->end();
       itr != last;
       itr++) {
    std::vector<torrent::vm_mapping> tmp_mappings =
      torrent::chunk_list_mapping((*itr)->download());

    all_mappings.insert(
      all_mappings.end(), tmp_mappings.begin(), tmp_mappings.end());
  }

  FILE* log_file = fopen(str.c_str(), "w");

  for (std::vector<torrent::vm_mapping>::iterator itr  = all_mappings.begin(),
                                                  last = all_mappings.end();
       itr != last;
       itr++) {
    fprintf(log_file,
            "%8p-%8p [%5llxk]\n",
