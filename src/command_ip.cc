...
bool
ipv4_range_parse(const char* address,
...
ipv4_range_parse(const char* address,
  ...
  *address_end   = 0;
  ...
  while (address[address_start_index] != '#' &&
         address[address_start_index] != '\r' &&
         address[address_start_index] != '\n' &&
         address[address_start_index] != '\0' && address_start_index < 4096) {
  ...
  while (((addr[0] >= '0' && addr[0] <= '9') || addr[0] == '.') &&
         address_start_index < 19) {
  ...
  if (strchr(addr, '-') != NULL) {
    ...
    if (inet_pton(AF_INET, address_start_str, &(sa_start.sin_addr)) != 0 &&
        inet_pton(AF_INET, address_end_str, &(sa_end.sin_addr)) != 0) {
    ...
    if (inet_pton(AF_INET, address_start_str, &(sa_start.sin_addr)) != 0 &&
        sscanf(mask_bits_str, "%u", &mask_bits) != 0) {
      if (mask_bits <= 32) {
...
ipv4_filter_parse(const char* address, int value) {
  ...
  if (ipv4_range_parse(address, &address_start, &address_end)) {
    ...
    lt_log_print(torrent::LOG_CONNECTION_FILTER,
                 "Adding ip filter for %s-%s.",
                 start_str,
                 end_str);
  }
}
...
apply_ipv4_filter_add_address(const torrent::Object::list_type& args) {
  ...
  ipv4_filter_parse(
    ...
    torrent::option_find_string(torrent::OPTION_IP_FILTER,
...
apply_ipv4_filter_load(const torrent::Object::list_type& args) {
  ...
  int         value =
    torrent::option_find_string(torrent::OPTION_IP_FILTER, value_name.c_str());

  std::fstream file(torrent::utils::path_expand(filename).c_str(),
                    std::ios::in);
  ...
  lt_log_print(torrent::LOG_CONNECTION_FILTER,
               "loaded %u %s address blocks (%u kb in-memory) from '%s'",
               lineNumber,
               value_name.c_str(),
...
apply_ipv4_filter_dump() {
  ...
  torrent::ipv4_table::range_map_type range_map =
    torrent::PeerList::ipv4_filter()->range_map;
...
initialize_command_ip() {
  CMD2_ANY_STRING(
    ...
    std::bind(&apply_ip_tables_insert_table, std::placeholders::_2));
  CMD2_ANY_STRING("ip_tables.size_data",
                  std::bind(&apply_ip_tables_size_data, std::placeholders::_2));
  CMD2_ANY_LIST("ip_tables.get",
                std::bind(&apply_ip_tables_get, std::placeholders::_2));
  CMD2_ANY_LIST("ip_tables.add_address",
                std::bind(&apply_ip_tables_add_address, std::placeholders::_2));

  CMD2_ANY("ipv4_filter.size_data", std::bind(&apply_ipv4_filter_size_data));
  CMD2_ANY_STRING("ipv4_filter.get",
                  std::bind(&apply_ipv4_filter_get, std::placeholders::_2));
  CMD2_ANY_LIST(
    "ipv4_filter.add_address",
    std::bind(&apply_ipv4_filter_add_address, std::placeholders::_2));
  CMD2_ANY_LIST("ipv4_filter.load",
                std::bind(&apply_ipv4_filter_load, std::placeholders::_2));
