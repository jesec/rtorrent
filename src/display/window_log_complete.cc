...
WindowLogComplete::redraw() {
  ...
  for (iterator itr = m_log->end(), last = m_log->begin();
    ...
    unsigned int logWidth  = m_canvas->width() > 60
                               ? (m_canvas->width() - timeWidth)
                               : (60 - timeWidth);
    ...
    for (unsigned int j = logHeight; j > 0 && pos > 0; --j, --pos)
      if (j == 1)
        m_canvas->print(
          ...
          "(%s) %s",
          buffer,
          itr->message.substr(0, m_canvas->width() - timeWidth).c_str());
      else
        m_canvas->print(
          timeWidth,
          pos - 1,
          "%s",
          itr->message.substr(logWidth * (j - 1), m_canvas->width() - timeWidth)
            .c_str());
  }
}
