...
SignalHandler::set_default(unsigned int signum) {
  if (signum > HIGHEST_SIGNAL)
    throw std::logic_error(
      "SignalHandler::set_default(...) received invalid signal value.");
...
SignalHandler::set_ignore(unsigned int signum) {
  if (signum > HIGHEST_SIGNAL)
    throw std::logic_error(
      "SignalHandler::set_ignore(...) received invalid signal value.");
...
SignalHandler::set_handler(unsigned int signum, slot_void slot) {
  if (signum > HIGHEST_SIGNAL)
    throw std::logic_error(
      "SignalHandler::set_handler(...) received invalid signal value.");

  if (!slot)
    throw std::logic_error(
      "SignalHandler::set_handler(...) received an empty slot.");
...
SignalHandler::caught(int signum) {
  if ((unsigned)signum > HIGHEST_SIGNAL)
    throw std::logic_error("SignalHandler::caught(...) received invalid signal "
                           "from the kernel, bork bork bork.");

  if (!m_handlers[signum])
    throw std::logic_error("SignalHandler::caught(...) received a signal we "
