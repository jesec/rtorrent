...
#ifndef RTORRENT_UTILS_JSONRPC_DISPATCHER_H
#define RTORRENT_UTILS_JSONRPC_DISPATCHER_H
...
public:
  ...
  JsonRpcException process_type_error(const std::string& name,
                                      JsonRpcException&  e) {
  ...
  json InvokeMethod(const std::string& name, const json& params) {
    ...
    try {
      ...
      throw JsonRpcException(-32602,
                             "invalid parameter: " + std::string(e.what()));
...
private:
  ...
  inline bool contains(const std::string& name) {
    return (methods.find(name) != methods.end() ||
            notifications.find(name) != notifications.end());
  }
    ...
    if (params.type() == json::value_t::array) {
      ...
      for (auto const& p : mapping[name]) {
        if (params.find(p) == params.end()) {
          throw JsonRpcException(
            -32602, "invalid parameter: missing named parameter \"" + p + "\"");
        }
