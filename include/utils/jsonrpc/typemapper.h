...
#ifndef RTORRENT_UTILS_JSONRPC_TYPEMAPPER_H
#define RTORRENT_UTILS_JSONRPC_TYPEMAPPER_H
...
template<typename T>
constexpr json::value_t
GetType(type<std::vector<T>>) {
...
constexpr json::value_t
GetType(type<long long>) {
  return json::value_t::number_integer;
}
constexpr json::value_t
GetType(type<unsigned short>) {
  return json::value_t::number_unsigned;
}
constexpr json::value_t
GetType(type<unsigned int>) {
  return json::value_t::number_unsigned;
}
constexpr json::value_t
GetType(type<unsigned long>) {
  return json::value_t::number_unsigned;
}
constexpr json::value_t
GetType(type<unsigned long long>) {
  return json::value_t::number_unsigned;
}

inline std::string
type_name(json::value_t t) {
  switch (t) {
...
check_param_type(
  size_t        index,
  const json&   x,
  json::value_t expectedType,
  typename std::enable_if<std::is_arithmetic<T>::value>::type* = 0) {
  if (expectedType == json::value_t::number_unsigned &&
      x.type() == json::value_t::number_integer) {
    if (x.get<long long int>() < 0)
      throw JsonRpcException(-32602,
                             "invalid parameter: must be " +
                               type_name(expectedType) + ", but is " +
                               type_name(x.type()),
                             index);
  } else if (x.type() == json::value_t::number_unsigned &&
             expectedType == json::value_t::number_integer) {
    if (x.get<long long unsigned>() > std::numeric_limits<T>::max()) {
      throw JsonRpcException(-32602,
                             "invalid parameter: exceeds value range of " +
...
MethodHandle
createMethodHandle(std::function<ReturnType(ParamTypes...)> method,
  ...
  MethodHandle handle = [method](const Parameters& params) -> json {
    ...
    (check_param_type<typename std::decay<ParamTypes>::type>(
       index,
       params[index],
       GetType(type<typename std::decay<ParamTypes>::type>())),
     ...);
    return method(
      params[index].get<typename std::decay<ParamTypes>::type>()...);
  };
  return handle;
}
...
MethodHandle
methodHandle(std::function<ReturnType(ParamTypes...)> method) {
  return createMethodHandle(method, std::index_sequence_for<ParamTypes...>{});
}

template<typename ReturnType, typename... ParamTypes>
MethodHandle
GetHandle(std::function<ReturnType(ParamTypes...)> f) {
  return methodHandle(f);
}
// Mapping for c-style function pointers
template<typename ReturnType, typename... ParamTypes>
MethodHandle
GetHandle(ReturnType (*f)(ParamTypes...)) {
...
NotificationHandle
createNotificationHandle(std::function<void(ParamTypes...)> method,
                         std::index_sequence<index...>) {
  NotificationHandle handle = [method](const Parameters& params) -> void {
    size_t actualSize = params.size();
    size_t formalSize = sizeof...(ParamTypes);
    // TODO: add lenient mode for backwards compatible additional params
    // if ((!allow_unkown_params && actualSize != formalSize) ||
    // (allow_unkown_params && actualSize < formalSize)) {
    if (actualSize != formalSize) {
      throw JsonRpcException(
        -32602,
        "invalid parameter: expected " + std::to_string(formalSize) +
...
NotificationHandle
notificationHandle(std::function<void(ParamTypes...)> method) {
  return createNotificationHandle(method,
                                  std::index_sequence_for<ParamTypes...>{});
}

template<typename... ParamTypes>
NotificationHandle
GetHandle(std::function<void(ParamTypes...)> f) {
  return notificationHandle(f);
}

template<typename... ParamTypes>
NotificationHandle
GetHandle(void (*f)(ParamTypes...)) {
...
MethodHandle
methodHandle(ReturnType (T::*method)(ParamTypes...), T& instance) {
...
NotificationHandle
notificationHandle(void (T::*method)(ParamTypes...), T& instance) {
  std::function<void(ParamTypes...)> function =
    [&instance, method](ParamTypes&&... params) -> void {
    return (instance.*method)(std::forward<ParamTypes>(params)...);
  };
...
template<typename T, typename ReturnType, typename... ParamTypes>
MethodHandle
GetHandle(ReturnType (T::*method)(ParamTypes...), T& instance) {
  std::function<ReturnType(ParamTypes...)> function =
    [&instance, method](ParamTypes&&... params) -> ReturnType {
    return (instance.*method)(std::forward<ParamTypes>(params)...);
  };
...
template<typename T, typename... ParamTypes>
NotificationHandle
GetHandle(void (T::*method)(ParamTypes...), T& instance) {
  std::function<void(ParamTypes...)> function =
    [&instance, method](ParamTypes&&... params) -> void {
    return (instance.*method)(std::forward<ParamTypes>(params)...);
  };
